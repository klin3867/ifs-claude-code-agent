...
# =============================================================================
# TOOL CATALOG - For system prompt injection
# =============================================================================

def get_tool_catalog(compact: bool = True) -> str:
    """
    Generate tool catalog for system prompt.

    Args:
        compact: If True, ~1,500 tokens (name + summary only).
                 If False, ~2,400 tokens (includes use_when hints).

    Lists all 59 tools organized by category.
    The model uses this to decide which tools to load via MCPSearch.
    """
    lines = ["## IFS Tools (use MCPSearch to load)"]
    ...
    category_order = ["system", "customers", "parts", "orders", "inventory",
                      "reservations", "shipments", "planning", "exceptions"]
    ...
    for category in category_order:
        ...
        for tool in sorted(tools, key=lambda t: t.name):
            flag = "!" if tool.mutates else ""
            if compact:
                lines.append(f"- {tool.name}{flag}: {tool.summary}")
            else:
                lines.append(f"- {tool.name}{flag}: {tool.summary} ({tool.use_when})")

    return "\n".join(lines)


def search_tools_by_keywords(query: str, top_k: int = 10) -> List[ToolSummary]:
    """
    Search tools by keyword overlap (no ML dependencies).

    Scores tools based on word overlap between query and:
    - tool name
    - summary
    - use_when
    - category
    ...
    Args:
        query: Search query (e.g., "check inventory stock levels")
        top_k: Maximum tools to return

    Returns:
        List of ToolSummary objects, sorted by relevance
    """
    query_words = set(query.lower().split())

    # Also add common word variations
    expanded = set()
    for word in query_words:
        expanded.add(word)
        # Handle common variations
        if word.endswith('s'):
            expanded.add(word[:-1])  # orders -> order
        else:
            expanded.add(word + 's')  # order -> orders
        if word.endswith('ing'):
            expanded.add(word[:-3])  # shipping -> ship
            expanded.add(word[:-3] + 'ment')  # shipping -> shipment

    scored = []
    for tool in TOOL_REGISTRY.values():
        # Build searchable text
        searchable = f"{tool.name} {tool.summary} {tool.use_when} {tool.category}".lower()
        searchable_words = set(searchable.replace('_', ' ').split())

        # Score by overlap
        overlap = len(expanded & searchable_words)
        ...
        if tool.category.lower() in query.lower():
            overlap += 2
...
# =============================================================================
# LEGACY: MCPToolRetriever (sentence-transformers based)
# Moved to LEGACY folder - keeping stub for backward compatibility
# =============================================================================

class MCPToolRetriever:
    """
    DEPRECATED: Use search_tools_by_keywords() instead.

    This class previously used sentence-transformers for semantic search.
    Now falls back to keyword-based search to avoid ~2GB dependencies.
    """

    _instance = None

    def __init__(self, model_path: str = None):
        print("MCPToolRetriever: Using keyword-based search (sentence-transformers removed)")
        print(f"MCPToolRetriever: Indexed {len(TOOL_REGISTRY)} tools")

    @classmethod
    def get_instance(cls, model_path: str = None) -> "MCPToolRetriever":
    ...
    def retrieve(self, query: str, top_k: int = 10) -> List[str]:
        """Retrieve tool names using keyword search."""
        tools = search_tools_by_keywords(query, top_k)
        return [t.name for t in tools]

    def retrieve_with_scores(self, query: str, top_k: int = 10) -> List[tuple]:
        """Retrieve tools with placeholder scores."""
        tools = search_tools_by_keywords(query, top_k)
        # Return decreasing scores for compatibility
        return [(t.name, 1.0 - i * 0.05) for i, t in enumerate(tools)]
